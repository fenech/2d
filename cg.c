...
void conjgrad(double **fgrid, int **lock, int maxits, double *fret, FILE * fp, const char * suffix, const t_par * par, int sep)
{
	...
	for (k = 1; k <= maxits; ++k) {
	    ...
	    */
	    ...
	    if (pass == 2) { /* zero the components of h from cells immediately surrounding the particles */		
		for (j = 35; j <= 65; ++j) {
		    for (i = nx / 2 - 2 * par[0].major - sep / 2; i <= nx / 2 - sep / 2; ++i) h[j][i] = 0;
		    for (i = nx / 2 + sep / 2; i <= nx / 2 + 2 * par[1].major + sep / 2; ++i) h[j][i] = 0;
		}
	    }
	    /*
	    ...
	    */
	    ...
	    if(strike == 50) {
		fprintf(fp, "#exiting pass %d after %d iterations\n", pass, k);
