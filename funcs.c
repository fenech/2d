...
inline static double frank(double **grid, int ni[], int nj[])
{
    double xcell[3] = {
            grid[nj[1]][ni[0]],
            grid[nj[1]][ni[1]],
    ...
    double dsindxr = (sin(xcell[2]) - sin(xcell[1])),
    ...
    return ((K_1 / 2) * splay + (K_3 / 2) * bend) / (CELL_LEN * CELL_LEN);
}
...
double locfunc(double **grid, int x, int y, int flag, int rank, int num_procs)
{
    ...
    for (j = 0; j < 5; ++j) {
        ...
        for (i = 0; i <= 2; ++i) {
            if (ni[i] < 1) ni[i] += nx;
            if (ni[i] > nx) ni[i] -= nx;
            if (rank == num_procs - 1 && nj[i] > h + 1) nj[i] = h + 1;
