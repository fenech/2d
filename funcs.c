...
double locfunc(double **grid, int x, int y, int flag)
{
  ...
  for (ci = x - 1; ci <= x + 1; ++ci) {
    ...
    xcell[2] = grid[nj[1]][ni[2]];
    xcell[1] = grid[nj[1]][ni[1]];
    xcell[0] = grid[nj[1]][ni[0]];
    ycell[2] = grid[nj[2]][ni[1]];
    ycell[1] = grid[nj[1]][ni[1]];
    ycell[0] = grid[nj[0]][ni[1]];
